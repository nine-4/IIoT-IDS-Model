...
for fold, (train_idx, val_idx) in enumerate(kf.split(X)):
    ...
    overlaps = val_combined.apply(tuple, axis=1).isin(train_combined.apply(tuple, axis=1))
    num_overlaps = overlaps.sum()
    print(f"Fold {fold + 1}: Number of overlapping samples between training and validation sets: {num_overlaps}")
